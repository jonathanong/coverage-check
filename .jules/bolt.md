...
## 2026-05-31 - String split vs index-based parsing on data lines
**Learning:** Just like traversing the main file string with `indexOf('
')` is faster than `.split('
')`, using `.indexOf(',')` to parse the comma-delimited fields within each individual line (like `DA:` and `BRDA:` lines) avoids massive intermediate array allocations in loops that run millions of times.
**Action:** For performance-critical parsing loops running on large strings, always prefer manual index traversal and slicing over `.split()`.
