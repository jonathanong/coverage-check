import { forEachBufferLine } from "./line-scanner.mts";
...
export function lcovBufferToIstanbul(lcov: Buffer, stripPrefixes: string[]): IstanbulCoverage {
  ...
  // Optimization: Avoid decoding the full LCOV buffer to a UTF-8 string first.
  // We traverse the buffer directly and convert each line slice to UTF-8 on demand.
  forEachBufferLine(
    lcov,
    (line) => {
      const trimmedLine = line.endsWith("\r") ? line.slice(0, -1) : line;

      if (trimmedLine === "end_of_record") {
        flush();
        filePath = null;
        return;
      }
      if (trimmedLine.startsWith("SF:")) {
        flush();
        filePath = normalizeFilePath(trimmedLine.slice(3).trim(), stripPrefixes);
      ...
      if (trimmedLine.startsWith("DA:")) {
        const [lineNo, hits] = trimmedLine.slice(3).split(",", 2);
        const l = Number.parseInt(lineNo!, 10);
        const h = Number.parseInt(hits ?? "", 10);
        if (!Number.isInteger(l) || !Number.isInteger(h)) return;
        const key = String(l);
      ...
      if (trimmedLine.startsWith("FN:") || trimmedLine.startsWith("FNL:")) {
        const rest = trimmedLine.slice(trimmedLine.startsWith("FNL:") ? 4 : 3);
      ...
      if (trimmedLine.startsWith("FNDA:") || trimmedLine.startsWith("FNA:")) {
        const rest = trimmedLine.slice(trimmedLine.startsWith("FNA:") ? 4 : 5);
        const commaIdx = rest.indexOf(",");
        if (commaIdx === -1) return;
        const h = Number.parseInt(rest.slice(0, commaIdx), 10);
        const name = rest.slice(commaIdx + 1);
        if (!Number.isInteger(h) || !name) return;
      ...
      if (trimmedLine.startsWith("BRDA:")) {
        const parts = trimmedLine.slice(5).split(",", 4);
        const lineNo = Number.parseInt(parts[0]!, 10);
        const blockId = parts[1] ?? "";
        const branchId = parts[2] ?? "";
        const taken = parts[3] === "-" ? 0 : Number.parseInt(parts[3] ?? "", 10);
        if (!Number.isInteger(lineNo) || !blockId || !branchId || !Number.isInteger(taken)) return;
