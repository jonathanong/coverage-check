...
export function lcovBufferToIstanbul(lcov: Buffer, stripPrefixes: string[]): IstanbulCoverage {
  ...
  while (start < text.length) {
    ...
    if (line.startsWith("DA:")) {
      // Optimization: avoid split() array allocation on millions of lines
      const commaIdx = line.indexOf(",", 3);
      if (commaIdx === -1) continue;
      const l = Number.parseInt(line.slice(3, commaIdx), 10);
      const h = Number.parseInt(line.slice(commaIdx + 1), 10);
  ...
  for (const [fp, blocks] of fileBranches) {
    ...
    for (const [blockKey, branches] of blocks) {
      ...
      const lineNo = Number.parseInt(dashIdx !== -1 ? blockKey.slice(0, dashIdx) : blockKey, 10); // blockKey is always "N-blockId"
