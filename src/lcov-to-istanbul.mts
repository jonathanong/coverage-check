...
export function lcovBufferToIstanbul(lcov: Buffer, stripPrefixes: string[]): IstanbulCoverage {
  ...
  for (const [fp, blocks] of fileBranches) {
    ...
    for (const [blockKey, branches] of blocks) {
      // Optimization: use indexOf instead of split
      const dash = blockKey.indexOf("-");
      const lineNo = Number.parseInt(dash === -1 ? blockKey : blockKey.slice(0, dash), 10); // blockKey is always "N-blockId"
