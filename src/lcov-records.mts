...
function applyRecord(line: string, cov: FullFileCoverage): void {
  if (line.startsWith("FN:")) {
    ...
    // Optimization: Manual index-based traversal avoids .split() array allocations
    // for massive LCOV files, significantly reducing GC overhead.
    ...
    if (comma3 === -1) return;
    if (line.indexOf(",", comma3 + 1) !== -1) return;
